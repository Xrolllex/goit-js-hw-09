...
const createPromise = (position, delay)=>{
    ...
    if (shouldResolve) console.log(`✅ Fulfilled promise ${position} in ${delay}ms`);
    else console.log(`❌ Rejected promise ${position} in ${delay}ms`);
};
const task = (event)=>{
    ...
    setTimeout(()=>{
        ...
        timer = setInterval(()=>{
            ...
            createPromise(meter, delay);
            if (meter > Number(input[2].value) - 1) clearInterval(timer);
        }, input[1].value);
    }, input[0].value);
};
